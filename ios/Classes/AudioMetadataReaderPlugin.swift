 ...
 *
 * Exposes methods to pick files/directories, start/stop accessing security-scoped resources,
 * and persist/restore security-scoped folder access (bookmarks) across app sessions on iOS.
 */
public class AudioMetadataReaderPlugin: NSObject, FlutterPlugin, UIDocumentPickerDelegate {
  private static let bookmarkStoreKey = "audio_metadata_reader.directoryBookmarks"
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(name: "audio_metadata_reader", binaryMessenger: registrar.messenger())
    let instance = AudioMetadataReaderPlugin()
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "debugInfo":
      result([
        "plugin": "AudioMetadataReaderPlugin",
  ...
  private func makeWorkingCopy(for sourceURL: URL) -> URL? {
    let tempDirectory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
      .appendingPathComponent("audio_metadata_reader_working", isDirectory: true)
